...
export default function PurchaseOrderPage() {
  ...
  return payload
        ...
        const {mutate} = useMutation({mutationFn:createPurchaseOrder,mutationKey:['invoices']})  
  ...
  return <span>${total.toFixed(2)}</span>
    ...
    function handleSubmit(data:PurchaseOrderScheme){
        if(status == 'save'){
            ...
            mutate({data:finalData},{onSuccess:(res)=>{toast(res.message,{position:'top-right',duration:3000})
